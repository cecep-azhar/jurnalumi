...
1. **`Note/task-qa.md`** — daftar tugas yang SEDANG dikerjakan (gate untuk publish). Ini yang menentukan task apa yang Anda kerjakan run ini. **File ini yang Anda update setiap run.**
...
5. `Note/prompt.md` — Aturan Keras asli (dirangkum ulang & diperkuat di bawah, tapi baca versi lengkapnya kalau ragu).
...
git status
```
- Kalau ada working tree kotor / branch aneh yang bukan hasil run ini (mis. kerjaan manual Prof yang belum di-commit) — **JANGAN** timpa atau reset. Stash dengan pesan jelas, catat di log, lanjut dengan hati-hati.
- Kalau ada branch `qa/*` yang masih ada dari run sebelumnya dan belum ke-PR — cek dulu apakah itu run yang crash di tengah jalan (lihat langkah 6).
...
```
- **Kalau ada ≥ 3 PR `qa/*` yang masih open (belum di-merge/di-close Prof): STOP.** Jangan mulai item baru. Tulis di log eksekusi (`task-qa.md`) bahwa run ini idle karena menunggu Prof review PR yang menumpuk, sebutkan nomor PR-nya, lalu selesai (exit 0, bukan error).
...
```
- Kalau `state=MERGED`: item itu **selesai**. Update `task-qa.md` (langsung di `main`, lihat langkah 4 soal cara commit-nya) jadi `[x]`, dan centang juga di `task.md` kalau item yang sama ada di sana.
...
> **Aturan keras baru (penyebab insiden 9 Sep 2026):** `Note/task-qa.md` dan `Note/task.md` **TIDAK PERNAH** ikut ter-commit di dalam branch `qa/*`. Update status HANYA lewat commit kecil terpisah, langsung di `main`. Kalau tidak, file tracker ikut hilang/nyasar setiap kali pindah branch, dan progress antar-item jadi tidak sinkron.
...
Di branch `qa/*`, **jangan sentuh `Note/task-qa.md` atau `Note/task.md` sama sekali** — commit di branch ini isinya cuma kode (+ test) untuk item yang sedang dikerjakan.
...
```
git add <file-yang-relevan-saja>    # jangan git add -A — cegah Note/task-qa.md ikut ke-add
...
**JANGAN merge PR sendiri. JANGAN push ke `main` selain commit tracker di langkah 4/9.**

### 9. Update dokumentasi — kembali ke `main`, commit terpisah lagi
```
git checkout main && git pull origin main
```
- `Note/task-qa.md`: item ini tetap `[~]` (PR baru dibuka, belum di-merge — lihat Legend), isi kolom Status dengan link PR yang baru dibuat.
- Kalau item yang sama juga ada di `Note/task.md` (banyak yang overlap by design): **jangan** centang `[x]` di sana juga sampai PR benar-benar merged (konsisten dengan aturan `[x]`=merged).
- Kalau nemu bug/mock baru di luar scope task ini selagi kerja: catat di section **"Temuan Baru Selama QA"** di `task-qa.md`, JANGAN diam-diam diperbaiki di luar task yang sedang jalan (kecuali 1-2 baris trivial yang memang bagian tak terpisahkan dari task ini).
- Tambah entri baru di **"Log Eksekusi"** — jangan timpa entri lama.
```
git add Note/task-qa.md
